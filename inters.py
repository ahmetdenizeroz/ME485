        ...
        def compute_delu(i_begin, i_end, *uf):
            for idx in range(i_begin, i_end):
                ...
                for jdx in range(nvars):
                    # Retrieve face values from the left and right elements
                    ul = uf[lti][lfi, jdx, lei] # Left face value
                    ur = uf[rti][rfi, jdx, rei] # Right face value
                    
                    # Compute the difference of face values
                    uf[lti][lfi, jdx, lei] = ul - ur
        ...
        def compute_avgu(i_begin, i_end, *uf):
            for idx in range(i_begin, i_end):
                ...
                for jdx in range(nvars):
                    ...
                    # **************************#
                    avg_value = 0.5 * (ul + ur) 
                    # You can store this average value in an appropriate location
                    # For example, updating a result array or storing in `uf` for later use:
                    uf[lti][lfi, jdx, lei] = avg_value  # Storing on the left element !!!! Bu uf store olayı sıkıntı yapar mı.
                                                        # Üsttekiyle make_delu ile çakışma
    ...
    def _make_delu(self):
        ...
        def compute_delu(i_begin, i_end, *uf):
            for idx in range(i_begin, i_end):
                ...
                # Compute the difference between left and right face values
                difference = ul - ur

                # Store the difference on the left element's corresponding face
                uf[lti][lfi, :, lei] = difference
    ...
    def _make_avgu(self):
        ...
        def compute_avgu(i_begin, i_end, *uf):
            for idx in range(i_begin, i_end):
                ...
                ul = uf[lti][lfi, :, lei]
                bc(ul, ur, nfi)
                # Compute the average face value
                avg_value = (ul + ur) / 2

                # Store the average value back into the left element's corresponding face
                uf[lti][lfi, :, lei] = avg_value  # !!!!! Bu storing sıkıntı yapabilir mi ? Üsttekiyle make_delu ile çakışma
